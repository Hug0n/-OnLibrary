...
echo '<a href="#" class="list-group-item text" style=border-radius:5px>';
    ...
    $sqlCurtidas = $Post->getQtdCurtidas($registroPosts['id_post']); //$SQL Curtidas

    $resultado_curtidas = mysqli_query($conn, $sqlCurtidas);
    ...
    $sqlPostCurtido = $Post->getPostCurtido($id_usuario, $registroPosts['id_post']); //$SQL Curtidas

    $esta_seguindo_usuario_tf = mysqli_query($conn, $sqlPostCurtido);
